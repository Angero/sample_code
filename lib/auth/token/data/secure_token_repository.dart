...
class SecureTokenRepository implements ITokenRepository {
  final FlutterSecureStorage datasource;

  SecureTokenRepository(this.datasource);
  ...
  Future<TokenEntity> readTokens() async {
    String accessToken = await datasource.read(key: ITokenDatasource.KEY_ACCESS_TOKEN);
    String refreshToken = await datasource.read(key: ITokenDatasource.KEY_REFRESH_TOKEN);
    return TokenEntity(
  ...
  Future<void> writeTokens(TokenEntity tokenEntity) async {
    await datasource.write(key: ITokenDatasource.KEY_ACCESS_TOKEN, value: tokenEntity.accessToken);
    await datasource.write(key: ITokenDatasource.KEY_REFRESH_TOKEN, value: tokenEntity.refreshToken);
  }
}